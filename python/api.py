...
trending_queries = [
    # DJ and Electronic Music Focus
    "popular DJ tracks 2024", "house music hits", "techno tracks trending", "EDM festival anthems",
    "deep house popular", "progressive house tracks", "trance music hits", "drum and bass popular",
    "dubstep drops", "future bass tracks", "tropical house hits", "tech house popular",
    "minimal techno tracks", "hardstyle anthems", "psytrance hits", "ambient electronic",
    
    # Popular DJ Artists
    "calvin harris", "david guetta", "martin garrix", "tiesto", "armin van buuren",
    "deadmau5", "skrillex", "diplo", "zedd", "marshmello", "alan walker", "kygo",
    "swedish house mafia", "above beyond", "carl cox", "charlotte de witte",
    
    # Popular Tracks (Mix of Electronic and Mainstream)
    "blinding lights", "levitating", "stay", "heat waves", "as it was",
    "bad habits", "flowers", "unholy", "anti hero", "watermelon sugar",
    
    # Genre-specific
    "electronic music 2024", "house music charts", "techno underground", "trance classics",
    "EDM top 40", "dance music hits", "club music popular", "festival tracks 2024",
    "remix popular songs", "electronic covers", "DJ mix tracks", "dance floor hits"
]
...
def download_with_ytdlp(url, output_path, title, artist):
    ...
    try:
        ydl_opts = {
            # Updated format selection for YouTube's new restrictions
            'format': 'bestaudio[acodec!=none]/best[acodec!=none]/bestaudio/best',
            'outtmpl': output_path,
            'noplaylist': True,
            'extractaudio': True,
            'audioformat': 'mp3',  # Direct MP3 extraction
            ...
            try:
                ...
                if 'Requested format is not available' in str(download_exception):
                    ...
                    try:
                        ...
                        print(f"❌ Retry also failed: {str(retry_exception)}")
...
def download_with_ytdlp_enhanced(url, output_path, title, artist, download_id):
    ...
    try:
        ...
        ydl_opts = {
            ...
            # Enhanced options for YouTube's new restrictions
            'extractor_retries': 5,
            'fragment_retries': 5,
            'retries': 5,
            'socket_timeout': 30,
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            try:
                ...
                # Try with different extractor options if first attempt fails
                if 'Requested format is not available' in str(download_exception):
                    print("🔄 Retrying with Android client only...")
                    ydl_opts['extractor_args']['youtube']['player_client'] = ['android']
                    ...
                    try:
                        ...
                        emit_progress(download_id, {
                            'stage': 'error',
                            'progress': 0,
                            'message': f'Download failed: {str(retry_exception)}',
                            'timestamp': time.time()
                        })
...
def stream_youtube_audio(video_id):
    ...
    try:
        ...
        # Get stream URL using yt-dlp with updated format selection
        ydl_opts = {
            'format': 'bestaudio[acodec!=none]/best[acodec!=none]/bestaudio/best',
            'quiet': True,
            'no_warnings': True,
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                try:
                    ...
                    return jsonify({"error": f"Could not extract video information: {str(e2)}"}), 404
            
            # Check if info extraction was successful
            if not info:
                return jsonify({"error": "Could not extract video information"}), 404
            
            # Get the stream URL directly from the selected format
            ...
            if not stream_url:
                ...
                if formats:
                    # Get the first available format (yt-dlp already selected the best)
                    stream_url = formats[0].get('url')
            
            if not stream_url:
                return jsonify({"error": "No audio stream URL available"}), 404
...
def youtube_download_queued():
    ...
    try:
        ...
        if not download_path:
            # Use default download path if none provided
            home_dir = os.path.expanduser("~")
            download_path = os.path.join(home_dir, "Music", "CAMELOTDJ")
            print(f"⚠️ No download path provided, using default: {download_path}")
