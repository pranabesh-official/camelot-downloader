...
const YouTubeMusic: React.FC<YouTubeMusicProps> = ({
    ...
    const [initialContentType, setInitialContentType] = useState<string>('');
    ...
    useEffect(() => {
                ...
                fetch(`http://127.0.0.1:${apiPort}/hello?signingkey=${apiSigningKey}`)
                    .then(response => {
                        if (response.ok) {
                            ...
                            setTimeout(() => {
                                if (socketRef.current === null) { // Only retry if not already connected
                                    createWebSocket();
                                }
                            }, 2000);
                        }
        ...
        // Load popular DJ tracks and user preferences on startup
        const loadInitialContent = async () => {
            try {
                setIsLoadingInitialContent(true);
                
                // Check for user preferences in localStorage
                const userPreferences = localStorage.getItem('user_music_preferences');
                let preferredGenres = ['electronic', 'house', 'techno', 'EDM'];
                
                if (userPreferences) {
                    try {
                        const prefs = JSON.parse(userPreferences);
                        preferredGenres = prefs.genres || preferredGenres;
                    } catch (e) {
                        console.log('Using default preferences');
                    }
                }
                
                // Popular DJ and electronic music queries based on preferences
                const djQueries = [
                    `${preferredGenres[0]} DJ tracks 2024`,
                    `popular ${preferredGenres[1]} music`,
                    `${preferredGenres[2]} hits trending`,
                    'DJ mix popular tracks',
                    ...
                    'trance music hits',
                    'drum and bass popular'
                ];
                
                // Pick a random query for variety
                const randomQuery = djQueries[Math.floor(Math.random() * djQueries.length)];
                
                console.log('🎧 Loading initial content:', randomQuery);
                setInitialContentType(`Popular ${randomQuery.split(' ')[0]} Music`);
                
                // Set the search query to show what we're loading
                ...
                await handleSearch('popular DJ tracks');
            } finally {
        ...
        // Load popular content after a short delay to ensure API is ready
        setTimeout(() => {
            loadInitialContent();
        }, 2000);
    ...
    // Save search to recent searches and update preferences
    const saveRecentSearch = useCallback((query: string) => {
        ...
        const musicGenres = ['house', 'techno', 'trance', 'dubstep', 'drum and bass', 'electronic', 'EDM', 'progressive', 'deep house', 'ambient', 'chillout'];
        const lowerQuery = trimmedQuery.toLowerCase();
        
        const detectedGenres = musicGenres.filter(genre => lowerQuery.includes(genre));
        
        if (detectedGenres.length > 0) {
            try {
                const currentPrefs = JSON.parse(localStorage.getItem('user_music_preferences') || '{"genres": ["electronic", "house", "techno", "EDM"]}');
                
                // Add detected genres to preferences (keep unique)
                const updatedGenres = [...new Set([...detectedGenres, ...currentPrefs.genres])].slice(0, 8);
                ...
                localStorage.setItem('user_music_preferences', JSON.stringify(updatedPrefs));
                console.log('🎵 Updated music preferences:', updatedGenres);
            } catch (e) {
            ...
            if (result.status === 'success') {
                ...
                if (result.tracks?.length === 0) {
                    if (isLoadingInitialContent) {
                        // If this was initial content loading, try a different query
                        console.log('🔄 No results for initial content, trying fallback...');
                        setTimeout(() => {
                            handleSearch('popular music 2024');
                        }, 1000);
                    } else {
    ...
    const createAudioElement = useCallback(() => {
        ...
        audio.onerror = (error) => {
            ...
            if (!isStoppingRef.current) {
                ...
                const retryDelay = Math.min(2000 * Math.pow(2, (audioState.error ? 1 : 0)), 10000);
                setTimeout(() => {
                    if (audioRef.current && currentlyPlayingId && !isStoppingRef.current) {
    ...
    const togglePlayPause = useCallback(async (track: YouTubeTrack) => {
        try {
            ...
            // If paused, resume
            if (currentlyPlayingId === track.id && !audioState.isPlaying && audioRef.current) {
                try {
    ...
    const handleProgressMouseDown = useCallback((e: React.MouseEvent) => {
        ...
        // Show preview while dragging starts
        seekTo(newTime, true);
    ...
    return (
        <div className="youtube-music-container" style={{ padding: 'var(--space-lg)' }}>
            ...
            {searchResults.length === 0 && !isSearching && !isLoadingInitialContent && (
                <div style={{
                    textAlign: 'center',
                    padding: '60px 20px',
                    color: 'var(--text-secondary)'
                    ...
                    <p style={{
                        ...
                        Search for your favorite DJ tracks, electronic music, or any songs you want to download and analyze.
                    </p>
                    <div style={{
                        display: 'flex',
                        flexWrap: 'wrap',
                        ...
                        {['Popular DJ Tracks', 'House Music', 'Techno Hits', 'EDM Charts', 'Deep House'].map(suggestion => (
                            <button
                                key={suggestion}
                    ...
                    <p style={{
                        ...
                        Or type in the search box above to find specific tracks
                    </p>
                </div>
            )}
            ...
            <div className="search-results">
                {(searchResults.length > 0 || isLoadingInitialContent) && (
                        ...
                        <h3 style={{ color: 'var(--text-primary)', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                            {isLoadingInitialContent ? (
                                <>
                                    ...
                                    Loading Popular DJ Tracks...
                                </>
                            ) : initialContentType && !searchQuery.includes('shape of you') ? 
                             `${initialContentType} (${filteredAndSortedResults.length})` :
